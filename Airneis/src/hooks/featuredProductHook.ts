...
export const useCreateFeaturedProductMutation = () => {
  return useMutation<FeaturedProduct, Error, FeaturedProduct>({
    mutationFn: async (featuredProduct: FeaturedProduct) => {
      const response = await apiClient.post<FeaturedProduct>(
...
export const useGetFeaturedProductsQuery = (): UseQueryResult<
  FeaturedProduct[],
  Error
> => {
  return useQuery({
    ...
    queryFn: async () => {
      const featuredProducts = (await apiClient.get('api/featuredProducts'))
        .data;
...
export const useUpdateFeaturedProductMutation = () => {
  return useMutation<FeaturedProduct, Error, {id: string; order: number}>({
    mutationFn: async ({id, order}) => {
      const response = await apiClient.put<FeaturedProduct>(
        `api/featuredProducts/${id}`,
        {order},
      );
      return response.data;
    },
