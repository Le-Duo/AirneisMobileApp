...
export const usePasswordResetRequestMutation = () => {
  return useMutation({
    mutationFn: async (email: string) => {
      return (await apiClient.post('api/users/password-reset-request', {email}))
        .data;
    },
  });
};
...
export const useUpdateUserMutation = (userId: string) =>
  useMutation({
    mutationFn: async (user: {
      ...
      (await apiClient.put<{user: UserInfo}>(`api/users/${userId}`, user)).data,
  });
  ...
  useMutation({
    mutationFn: async (cardId: string) => {
      const response = await apiClient.put<{}>(
        `api/users/${userId}/payment/card/${cardId}/default`,
      );
      return response.data;
    },
  });
